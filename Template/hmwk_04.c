...
int main( int argc, char *argv[] )
{
  ...
  //       space got allocated OK!)
	...
	projected_vertex = (Vertex *) calloc( 1, sizeof( Vertex ) );
	if ( projected_vertex == NULL ) 
	{
		fprintf( stderr, "allocView: Unable to allocate a place for the projected versions of the vertices.\n" );
		exit( 1 );
	}
  ...
  //       just allocated using projectVertexList2().
	projectVertexList2( p, m->m_vertex, m->m_numVertices, projected_vertex);
  //--------------------------------------
  ...
  // Remember, if the camera distance is 	.0 (representing a
  ...
  //       "infinity".
	...
	camera_pos.x = 0;
	camera_pos.y = 0;
	camera_pos.z = p->m_cameraDistance;
  ...
  //       this).
	...
	{
		int is_vis = 0;
		is_vis = isVisible(&camera_pos, &m->m_vertex[m->m_face[i].v1], &m->m_vertex[m->m_face[i].v2], &m->m_vertex[m->m_face[i].v3]);
		//printf("check : %d\n",is_vis);
		if (is_vis == 1)
		{	
